...
/** REPORTE 1 */ 
SELECT ciu.NOMBRE_CIU, cf.FECHA_CABECERA_FACTU, c.NOM_CLI, (SELECT SUM(PRECIO__DETALLE_COMPROBANTE_FACTURA) as PRECIO 
															FROM detalle_comprobante_factura df, cliente c, cabecera_factura cf, ciudad ciu
															WHERE df.NUMERO_CABECERA_FACTU = cf.NUMERO_CABECERA_FACTU AND cf.CODIGO_CLI = c.CODIGO_CLI AND c.CODIGO_CIU = ciu.CODIGO_CIU AND ciu.NOMBRE_CIU = ? AND 
															cf.FECHA_CABECERA_FACTU BETWEEN ? AND ?
															GROUP BY df.NUMERO_CABECERA_FACTU) AS DOLARES
